...
class CaptureEngine:
    ...
    def _capture_with_playwright(self, url: str, output_path: str, max_retries: int, logger) -> bool:
        """Capture screenshot using Playwright safely via temp file.
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Write URL and output path to a JSON file to avoid shell injection
...
try:
    with sync_playwright() as p:
        ...
        page.goto(params['url'], timeout=params['timeout'], wait_until='networkidle')
        page.wait_for_timeout(1000)
        page.screenshot(path=params['output_path'], full_page=True)
        browser.close()
        sys.exit(0)
except Exception as e:
    ...
    def download_assets(self, html: str, base_url: str, output_dir: str) -> List[str]:
        ...
        for url in asset_urls:
            try:
                ...
                # Enforce max asset size
                content_length = len(response.content)
                if content_length > MAX_ASSET_SIZE_BYTES:
                    import logging
                    logging.getLogger(__name__).warning(
                        f"Asset {url} exceeds max size ({content_length} > {MAX_ASSET_SIZE_BYTES}), skipping"
                    )
                    continue
