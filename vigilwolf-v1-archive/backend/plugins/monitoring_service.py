...
class MonitoringService:
    ...
    def get_latest_ping_log(self, domain_id: str) -> Optional[Dict]:
        """Get the latest ping log entry for a domain.

        Args:
            domain_id: ID of the domain

        Returns:
            Dictionary with ping log data or None if no logs exist
        """
        entry = self.storage.get_latest_ping_log(domain_id)
        if entry is None:
            return None
        return {
    ...
    def trigger_force_dump(self, domain_id: str) -> Snapshot:
        ...
        lock_path = Path(self.storage.data_dir) / "snapshots" / domain_id / ".dump.lock"
        ...
        if FileLock is not None:
            ...
            # Fallback to racy file-based lock if filelock not installed
        ...
        try:
            ...
            if domain.dump_mode == "html_and_assets":
                try:
                    ...
                    logger.warning(f"Asset download failed for {domain.url}: {e}")
                    asset_count = 0
                    assets_dir = None
            ...
            logger.error(f"Unexpected error during dump for {domain.url}: {e}", exc_info=True)
            snapshot = Snapshot.create(
                domain_id=domain.id,
                trigger_type=trigger_type,
                html_path="",
    ...
    def get_snapshot_details(self, snapshot_id: str) -> Optional[Dict]:
        ...
        Returns:
            Dictionary containing snapshot details, or None if not found
        """
